...
function startHttpServer() {
  ...
  apiApp.get('/api/games', async (req, res) => {
    try {
      const games = await getAllGames()
      res.json(games)
    } catch (error) {
  ...
  apiApp.get('/api/games/:id', async (req, res) => {
    try {
  ...
  // POST /api/games - 创建游戏
  apiApp.post('/api/games', async (req, res) => {
    try {
      const games = await getAllGames()
  ...
  apiApp.put('/api/games/:id', async (req, res) => {
    try {
      const games = await getAllGames()
  ...
  apiApp.delete('/api/games/:id', async (req, res) => {
    try {
      const games = await getAllGames()
      const gameIndex = games.findIndex(game => game.id === req.params.id)
